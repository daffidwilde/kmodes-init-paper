...
PATH = Path("../../../data/matching_over_huang_final_cost/")
...
def difference_fitness(dataframe, n_clusters=3, seed=0):
    ...
    method. Return the difference between their final costs so as to minimise
    that of the matching method. """

    km_matching = KModes(n_clusters=n_clusters, init="matching",
            n_init=25, random_state=seed).fit(dataframe)
    km_huang = KModes(n_clusters=n_clusters, init="huang",
            n_init=25, random_state=seed).fit(dataframe)
