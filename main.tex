...
\begin{itemize}
	...
	\[
		X^{(i)} = [A_1 = x_1^{(i)}, A_2 = x_2^{(i)}, \ldots, A_m = 
				x_m^{(i)}], \ \ i=1, \ldots, N
	\]
		where $x_j^{(i)}$ is the value of the $j^{th}$ attribute of the $i^{th}$
        data point, $X^{(i)}$.
	\item Prototype-based clustering algorithms partition the elements of 
		$\textbf{X}$ into $k$ distinct sets (clusters) denoted by $C_1, \ldots, 
        C_k$, where $k \in \mathbb{Z}_+$ is a pre-determined, fixed integer such
        that $k \le N$. That is:
	\[
		C_1, \ldots, C_k \text{ are such that } \bigcup_{l=1}^k C_l = 
		\textbf{X} \text{ \ and \ } C_l \cap C_t = \emptyset 
		\text{ for all } l \ne t
	\]
