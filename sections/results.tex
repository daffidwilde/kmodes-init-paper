...
that a particular behaviour emerged during the \(k\)-modes runs. Namely, that
each solution falls into one of (predominantly) two types: effectively no
improvement on the initial clustering, or terminating at some clustering with a
practically unique cost across all such solutions. Invariably, Cao's method
achieves this lower bound and unless Cao's method is used, these particular
choices for \(k\) mean that the performance of the \(k\)-modes algorithm is
exceptionally sensitive to its initial clustering. Moreover, the other two
methods are effectively indistinguishable in these cases.
...
theoretically counter-intuitive \textcolor{red}{but may be seen as follows\ldots
Can a small example be thought up? \(k = 2\) is a nice place to start.}
Fundamentally, this is possible since the cluster centres are updated after
...
beneficial to the cluster overall in terms of within-cluster dissimilarity
