...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[0]
        ...
        C_k\)}
        \\
        \Procedure{Update}{$\mu^{(l)}$}\Comment{How to update a current mode}
        ...
        \State{\(\bar{\mu} \gets \left\{\mu^{(1)}, \ldots, 
            \mu^{(k)}\right\}\)}
        \\
        \For{\(X_i \in \textbf{X}\)}\Comment{First cluster assignment}
		...
		\EndFor
        \\
        \Repeat\Comment{Continue reassigning points to the appropriate cluster
            until no point moves}{%
            \For{\(X^{(i)} \in \textbf{X}\)}
                \For{\(\mu^{(l)} \in \bar{\mu}\)}
