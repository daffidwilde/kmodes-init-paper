...
\begin{example}\label{ex:cao}
    We will now find our \(k\) initial modes using Cao's method, as we did in
    Example~\ref{ex:huang}. We begin by calculating the average density of each
    of our points \(X^{(i)} \in \textbf{X}, \ \text{Dens}(X^{(i)})\). These are
    given in Table~\ref{tab:density}.

    \begin{table}[h]
    \resizebox{\textwidth}{!}{%
        \input{tex/density-table.tex}
    }
    \end{table}
    \begin{table}[h]
    \resizebox{\textwidth}{!}{%
